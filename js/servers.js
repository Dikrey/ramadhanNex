 // ============================================
        ...
        const dailyQuotes = [
            { text: "Barangsiapa yang menghidupkan bulan Ramadan dengan iman dan mengharap pahala, maka diampunilah dosa-dosanya yang telah lalu.", source: "HR. Bukhari & Muslim" },
            { text: "Puasa itu perisai, maka apabila salah seorang di antara kalian berpuasa, janganlah ia berkata keji dan berteriak-teriak.", source: "HR. Bukhari" },
            { text: "Sebaik-baik amal perbuatan adalah shalat pada waktunya.", source: "HR. Bukhari & Muslim" },
            { text: "Sedekah itu dapat menghapus dosa sebagaimana air memadamkan api.", source: "HR. Tirmidzi" },
            { text: "Barangsiapa memberi makan orang yang berpuasa, maka baginya pahala seperti pahala orang yang berpuasa tanpa dikurangi sedikitpun.", source: "HR. Tirmidzi" }
        ];

        const duas = [
            { title: "Doa Buka Puasa", arabic: "اَللّٰهُمَّ لَكَ صُمْتُ وَبِكَ آمَنْتُ وَعَلَى رِزْقِكَ أَفْطَرْتُ", latin: "Allahumma laka shumtu wa bika amantu wa 'ala rizqika afthartu", meaning: "Ya Allah, untuk-Mu aku berpuasa, kepada-Mu aku beriman, dan dengan rezeki-Mu aku berbuka." },
            { title: "Doa Sahur", arabic: "بِسْمِ اللهِ وَعَلَى بَرَكَةِ اللهِ", latin: "Bismillahi wa 'ala barakatillah", meaning: "Dengan nama Allah dan dengan berkah Allah." },
            { title: "Doa Setelah Makan", arabic: "اَلْحَمْدُ لِلّٰهِ الَّذِيْ أَطْعَمَنَا وَسَقَانَا وَجَعَلَنَا مُسْلِمِيْنَ", latin: "Alhamdulillahilladhi ath'amana wa saqana wa ja'alana muslimin", meaning: "Segala puji bagi Allah yang telah memberi makan dan minum kepada kami, serta menjadikan kami orang-orang Islam." },
            { title: "Doa Masuk Masjid", arabic: "اَللّٰهُمَّ افْتَحْ لِيْ أَبْوَابَ رَحْمَتِكَ", latin: "Allahumma iftah li abwaba rahmatik", meaning: "Ya Allah, bukakanlah untukku pintu-pintu rahmat-Mu." },
            { title: "Doa Keluar Masjid", arabic: "اَللّٰهُمَّ إِنِّيْ أَسْأَلُكَ مِنْ فَضْلِكَ", latin: "Allahumma inni as'aluka min fadlik", meaning: "Ya Allah, sesungguhnya aku memohon kepada-Mu dari karunia-Mu." }
        ];

        const hadiths = [
            { text: "Rasulullah SAW bersabda: 'Barangsiapa yang mendirikan Lailatul Qadr karena iman dan mengharap pahala, maka diampunilah dosa-dosanya yang telah lalu.'", source: "HR. Bukhari & Muslim" },
            { text: "'Barangsiapa yang shalat malam di bulan Ramadan karena iman dan mengharap pahala, maka diampunilah dosa-dosanya yang telah lalu.'", source: "HR. Bukhari & Muslim" },
            { text: "'Barangsiapa yang memberi makan orang yang berpuasa, maka baginya pahala seperti pahala orang yang berpuasa tersebut, tanpa dikurangi sedikitpun dari pahalanya.'", source: "HR. Tirmidzi" },
            { text: "'Allah tidak menyia-nyiakan pahala orang yang berbuat kebaikan.'", source: "HR. Bukhari" },
        ...
        const achievements = [
            { id: 'first_streak', icon: '🔥', name: 'Api Pertama', desc: 'Streak pertama kali', unlocked: false },
            { id: 'streak_7', icon: '⚡', name: '7 Hari Berturut', desc: 'Streak 7 hari', unlocked: false },
            { id: 'streak_30', icon: '👑', name: 'Ramadan Master', desc: 'Streak 30 hari', unlocked: false },
            { id: 'quran_100', icon: '📖', name: 'Pembaca Setia', desc: 'Baca 100 halaman Quran', unlocked: false },
            { id: 'quran_khatam', icon: '📚', name: 'Khatam Quran', desc: 'Selesai khatam Quran', unlocked: false },
            { id: 'dzikir_1000', icon: '📿', name: 'Dzikir 1000x', desc: 'Total dzikir 1000x', unlocked: false },
            { id: 'sedekah_10', icon: '💝', name: 'Derawan', desc: 'Sedekah 10 kali', unlocked: false },
            { id: 'tarawih_100', icon: '🌙', name: 'Tarawih 100 Rakaat', desc: 'Total 100 rakaat tarawih', unlocked: false }
        ];
        ...
        function renderPrayerTimes() {
            ...
            grid.innerHTML = Object.entries(times).map(([name, time]) => {
                let iconClass = '';
                let bgColorClass = '';
                switch (name) {
                    case 'imsak': iconClass = 'fa-moon'; bgColorClass = 'bg-blue-100 text-blue-600'; break;
                    case 'subuh': iconClass = 'fa-cloud-sun'; bgColorClass = 'bg-indigo-100 text-indigo-600'; break;
                    case 'dzuhur': iconClass = 'fa-sun'; bgColorClass = 'bg-yellow-100 text-yellow-600'; break;
                    case 'ashar': iconClass = 'fa-cloud'; bgColorClass = 'bg-orange-100 text-orange-600'; break;
                    case 'maghrib': iconClass = 'fa-cloud-moon'; bgColorClass = 'bg-rose-100 text-rose-600'; break;
                    case 'isya': iconClass = 'fa-star'; bgColorClass = 'bg-purple-100 text-purple-600'; break;
                    default: iconClass = 'fa-mosque'; bgColorClass = 'bg-gray-100 text-gray-600';
                }
        ...
        function renderTargets() {
            ...
            const targetItems = [
                { id: 'sholat', label: 'Sholat 5 Waktu', icon: 'fa-mosque', color: 'emerald' },
                { id: 'quran', label: 'Baca Al-Quran', icon: 'fa-book-quran', color: 'amber' },
                { id: 'sedekah', label: 'Sedekah', icon: 'fa-heart', color: 'rose' },
                { id: 'tarawih', label: 'Sholat Tarawih', icon: 'fa-moon', color: 'indigo' },
                { id: 'dzikir', label: 'Dzikir Pagang & Sore', icon: 'fa-hands-praying', color: 'purple' },
                { id: 'sahur', label: 'Sahur', icon: 'fa-utensils', color: 'orange' },
                { id: 'buka', label: 'Buka Puasa Tepat Waktu', icon: 'fa-calendar-check', color: 'teal' },
                { id: 'tadarus', label: 'Tadarus Masjid', icon: 'fa-book-open', color: 'cyan' }
            ];

            list.innerHTML = targetItems.map(item => {
                const isDone = todayTargets[item.id]?.done || false;
                const timeInfo = todayTargets[item.id]?.time ? `<span class="text-[10px] text-slate-400">${todayTargets[item.id].time}</span>` : '';
                return `
        ...
        function renderWaterDrops() {
            ...
            document.getElementById('water-ml').textContent = (state.waterGlasses * 250) + ' ml';
            document.getElementById('water-count').textContent = state.waterGlasses + '/8';
        }
        ...
        function renderAsmaulHusna() {
            ...
            grid.innerHTML = asmaulHusna.map(a => `
        ...
        function renderDzikirList() {
            ...
            container.innerHTML = dzikirList.map(d => `
                <div class="glass-card p-4 rounded-2xl">
        ...
        function renderJournalEntries() {
            ...
            if (state.journals.length === 0) {
                container.innerHTML = '<p class="text-center text-slate-400 py-8">Belum ada journal. Mulai menulis!</p>';
                return;
            }
            container.innerHTML = state.journals.slice().reverse().map(j => `
        ...
        function renderQuranBookmarks() {
            ...
            if (state.quranBookmarks.length === 0) {
                container.innerHTML = '<p class="text-center text-slate-400 py-4">Belum ada bookmark</p>';
                return;
            }
            container.innerHTML = state.quranBookmarks.map(b => `
        ...
        function renderFastingCalendar() {
            ...
            let html = days.map(d => `<div class="text-center text-xs font-bold text-slate-400 py-2">${d}</div>`).join('');
        ...
        function renderDailyPlan() {
            ...
            const plan = [
                { time: times.subuh, title: "Sholat Subuh & Dzikir Pagi", icon: "fa-cloud-sun" },
                { time: "06:00", title: "Tadarus Al-Quran", icon: "fa-book-quran" },
                { time: times.dzuhur, title: "Sholat Dzuhur", icon: "fa-sun" },
                { time: times.ashar, title: "Sholat Ashar & Dzikir Sore", icon: "fa-cloud" },
                { time: times.maghrib, title: "Buka Puasa & Sholat Maghrib", icon: "fa-cloud-moon" },
        ...
        function updateNextPrayer(now) {
            ...
            for (const prayer of prayerOrder) {
                ...
                if (state.settings.prayerNotification && !state.notificationsShown[notifKey]) {
                    if (diff > 0 && diff <= 10) { // 10 minutes before
                        showPrayerNotification(`Waktu ${prayer} akan tiba dalam ${diff} menit.`, 'fa-clock');
                        state.notificationsShown[notifKey] = 'upcoming';
                    } else if (diff === 0) { // On time
                        showPrayerNotification(`Waktu ${prayer} telah masuk.`, 'fa-bell', true);
            ...
            document.getElementById('countdown').textContent = `Menuju ${nextPrayer.charAt(0).toUpperCase() + nextPrayer.slice(1)} (${nextTime})`;
            ...
            let maghribDate = new Date(now.getFullYear(), now.getMonth(), now.getDate(), maghribH, maghribM, 0);
        ...
        function updateRamadanProgress(now) {
            ...
            const day = Math.min(30, Math.max(1, Math.floor((now - ramadanStart) / (1000 * 60 * 60 * 24)) + 1));
        ...
        function toggleTarget(id) {
            ...
            if (state.targetHistory[today][id].done) {
                state.targetHistory[today][id].time = new Date().toLocaleTimeString('id-ID', { hour: '2-digit', minute: '2-digit' });
                addXP(10);
            } else {
        ...
        function updateDailyProgress() {
            const today = new Date().toDateString();
            const todayTargets = state.targetHistory[today] || {};
            const targetKeys = ['sholat', 'quran', 'sedekah', 'tarawih', 'dzikir', 'sahur', 'buka', 'tadarus'];
            ...
            document.getElementById('daily-progress-text').textContent = `${done} dari ${targetKeys.length} target selesai`;
        }
        ...
        function addQuranPages() {
            ...
            if (pages > 0) {
                ...
                state.targetHistory[today].quran = { done: true, pages: pages, time: new Date().toLocaleTimeString('id-ID') };
        ...
        function updateQuranUI() {
            ...
            const percent = Math.min(100, Math.round((state.quranPages / totalPages) * 100));
        ...
        document.getElementById('zakat-input').addEventListener('input', function() {
            ...
            if (val > 0) {
                ...
                document.getElementById('zakat-amount').textContent = 'Rp ' + zakat.toLocaleString('id-ID');
            } else {
        ...
        function addSedekah() {
            ...
            if (amount > 0) {
                ...
                const today = new Date().toDateString();
                if (!state.targetHistory[today]) state.targetHistory[today] = {};
                state.targetHistory[today].sedekah = { done: true, amount: amount, time: new Date().toLocaleTimeString('id-ID') };
        ...
        function updateSedekahUI() {
            document.getElementById('total-sedekah').textContent = 'Rp ' + state.sedekahTotal.toLocaleString('id-ID');
        }
            ...
            if (statsCtx) {
                ...
                charts.stats = new Chart(statsCtx, {
                    ...
                    options: {
                        ...
                        plugins: { legend: { display: false } },
                        scales: {
                            y: { beginAtZero: true, max: Math.max(...data, 1), ticks: { stepSize: 1 } }
                        }
                    }
                });
            }
        ...
        function openSettings() {
            ...
            document.getElementById('prayer-notification').checked = state.settings.prayerNotification;
            document.getElementById('sahur-reminder').checked = state.settings.sahurReminder;
        ...
        function saveSettings() {
            state.settings.name = document.getElementById('user-name').value || 'Pengguna';
            state.settings.location = document.getElementById('user-location').value;
            state.settings.prayerNotification = document.getElementById('prayer-notification').checked;
            state.settings.sahurReminder = document.getElementById('sahur-reminder').checked;
        ...
        function showPrayerDetails(element, name, time) {
            ...
            let prayerDate = new Date(now.getFullYear(), now.getMonth(), now.getDate(), prayerH, prayerM, 0);
        ...
        function calculateQiblaAngle(lat, lon) {
            ...
            const x = Math.cos(latRad) * Math.tan(kaabaLat) - Math.sin(latRad) * Math.cos(deltaLon);
                    ...
                    .then(permissionState => {
                        if (permissionState === 'granted') {
                            window.addEventListener('deviceorientation', handleOrientation);
                             ...
                             document.getElementById('qibla-degree').textContent = 'Izin orientasi perangkat ditolak.';
                        }
                    })
                    .catch(console.error);
            } else {
        ...
        if (qiblaNavItem) {
            qiblaNavItem.addEventListener('click', () => {
                if (!qiblaAngle) { // Only fetch if not already calculated
                    getQiblaDirection();
                }
            });
        }
        ...
        function updateUI() {
            ...
            document.getElementById('daily-quote').textContent = '"' + dailyQuotes[quoteIndex].text + '"';
            document.getElementById('quote-source').textContent = '- ' + dailyQuotes[quoteIndex].source;
        ...
        function getStreakChartData() {
            ...
            for (let i = 0; i < 7; i++) {
                ...
                const dayHistory = state.history.find(h => new Date(h.date).toDateString() === dateString);
                if (dayHistory) {
        ...
        function getMoodDistributionData() {
            ...
            return {
                ...
                data: [moodCounts['😊'], moodCounts['🙂'], moodCounts['😐'], moodCounts['😔']]
            };
        }
        ...
        function getWeeklyActivityData() {
            ...
            const totalPrayers = Object.values(state.targetHistory).reduce((acc, dailyTargets) => {
                return acc + Object.values(dailyTargets).filter(t => t.done && ['sholat', 'sahur', 'buka'].includes(t.id)).length;
            }, 0);
